...
hw=hw[,-c(1,7,20,22,31:35)] ##Since Regression Tree only allows maximum 32 factors, also, there are some useless columns 
...
summary (tree.hw)
## Variables actually used in tree construction:"max.temp.quantile" "Ppoverty"  "start.date"     
plot(tree.hw)
...
set.seed(1)
bag.hw=randomForest(Estimate~.,data=hw,subset=train,mtry=25,importance=TRUE)
yhat.bag=predict(bag.hw,newdata=hw[-train ,])
plot(yhat.bag,hw.test)
abline(0,1)
mean((yhat.bag - hw.test)^2)
##  MSE=0.03496675 even bigger than single tree.
importance(bag.hw)
varImpPlot(bag.hw)
```
